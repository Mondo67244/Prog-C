...
#include <stdio.h>
int main() {
  // Exo 2 : creer le fichier examen.txt et y ecrire quelques lignes de javae
  int i, n;
  char contenu[100];
  ...
  scanf("%d", &n);
  getchar();
  // Ecriture de quelque lignes de javae
  for (i = 0; i < n; i++) {
    printf("Entrez la phrase %d : ", i + 1);

    // fgets prend en parametre le recepteur ,la taille du recepteur , l'entree
    // / source
    fgets(contenu, 100, stdin); /*ecrire dans le tampon contenu en limit 100 et
 l'imput du clavier*/

    // fprintf prend en parametre le destinataire, le type de la source , la
    // source
